...
from src.constant.data_dict_dialog_constant import SYNC_DEFAULT_DATA_DICT_BTN_TEXT, DEL_DATA_DICT_BTN_TEXT, \
    ...
    QUERY_DATA_DICT_BIND_DATA_TITLE, NO_DATA_DICT_PROMPT, SORT_DATA_DICT_BUTTON_TEXT, DATA_DICT_SORT_BOX_TITLE
...
from src.view.dialog.data_dict.data_dict_sort_dialog import DataDictSortDialog
...
class DataDictDetailDialogFrame(SaveDialogFrame):
    ...
    def __init__(self, data_dict_type, *args):
        ...
        # 对字典项排序的对话框
        self.sort_dialog: DataDictSortDialog = ...
        # 处理需要重新绑定数据的数据字典项对话框
    ...
    def setup_content_ui(self):
        ...
        self.table_button_layout.addWidget(QLabel(), 0, 0, 1, 2)
        self.sync_default_values_button = QPushButton()
        self.sync_default_values_button.setObjectName('sync_default_values_button')
        self.table_button_layout.addWidget(self.sync_default_values_button, 0, 2, 1, 2)
        self.sort_button = QPushButton()
        self.sort_button.setObjectName('sort_button')
        self.table_button_layout.addWidget(self.sort_button, 0, 4, 1, 1)
        self.add_button = QPushButton()
    ...
    def setup_other_label_text(self):
        ...
        self.sort_button.setText(SORT_DATA_DICT_BUTTON_TEXT)
    ...
    def open_sort_dialog(self):
        data_dict_list = self.get_data_dict_list(DATA_DICT_SORT_BOX_TITLE)
        if data_dict_list:
            self.sort_dialog = DataDictSortDialog(data_dict_list)
            self.sort_dialog.save_signal.connect(self.rerender_table)
            self.sort_dialog.exec()

    def rerender_table(self, data_dict_list):
        self.table_widget.clearContents()
        self.table_widget.setRowCount(0)
        self.table_widget.fill_table(data_dict_list)

    def get_data_dict_list(self, box_title):
        data_dict_list = self.table_widget.collect_data()
        # 名称检查
        for data_dict in data_dict_list:
            if not data_dict.dict_name:
                pop_fail(BLANK_DATA_DICT_NAME_PROMPT, box_title, self.parent_dialog)
                return
    ...
    def save_func(self):
        data_dict_list = self.get_data_dict_list(DATA_DICT_DETAIL_BOX_TITLE)
        if data_dict_list:
            # 检查绑定数据是否需要重新分配
